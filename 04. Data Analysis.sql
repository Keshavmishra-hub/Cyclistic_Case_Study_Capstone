...
SELECT month, member_casual, AVG(tripduration) AS avg_ride_duration
FROM `my_project_id.2023_tripdata.combined_data_cleaned`
GROUP BY month, member_casual;

-- average ride_length per day of week

SELECT day_of_week, member_casual, AVG(tripduration) AS avg_ride_duration
FROM `my_project_id.2023_tripdata.combined_data_cleaned`
GROUP BY day_of_week, member_casual;

-- average ride_length per hour

SELECT EXTRACT(HOUR FROM started_at) AS hour_of_day, member_casual, AVG(tripduration) AS avg_ride_duration
FROM `my_project_id.2023_tripdata.combined_data_cleaned`
GROUP BY hour_of_day, member_casual;
